...
- **Pre-commit hooks** declared in a published `.pre-commit-hooks.yaml` — local-advisory checks consumed by downstream `.pre-commit-config.yaml` files. Hooks use `language: script` and delegate to thin wrapper scripts under `hooks/` that encode NVIDIA-approved args and resolve the scanner binary from `$PATH`. Pre-commit runs on developer machines only; consumers need the underlying scanner binary installed locally (per-tool install steps live in each wrapper's header).
...
The repository is currently an early scaffold (`v0.1.0` pre-release); no scan surface is published yet. Secret scanning is the first planned scan and will ship two lanes: a CI enforcement workflow (`secret-scan-pulse.yml` — NVIDIA-licensed Pulse Secret Scanner, run on `nv-gha-runners`) and a local-advisory pre-commit hook (open-source `trufflesecurity/trufflehog`). Pulse is container-only, so it is CI-only; the pre-commit lane deliberately uses the OSS CLI to stay within the <10s DX budget on developer machines. See [`ROADMAP.md`](ROADMAP.md) for per-scan status.
