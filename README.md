...
- **Fail-closed by default.** A finding blocks the merge unless an explicit, documented exception applies. **Documented exception — CodeQL SAST:** [`sast-scan-codeql.yml`](.github/workflows/sast-scan-codeql.yml) publishes findings to code scanning and always exits 0. CodeQL results are gated through code-scanning alert rules and branch protection rather than a workflow exit code, so enforcement for that scan is a repository-side setting, not an input.
